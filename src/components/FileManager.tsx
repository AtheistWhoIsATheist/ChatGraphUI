...
export function FileManager({ onExtract, files, setFiles, onClose }: FileManagerProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950 border-r-2 border-white/5 font-mono">
      {/* Header */}
      <div className="p-6 border-b-2 border-white/5 bg-zinc-900/40 relative z-10 shadow-xl shrink-0">
        ...
        {/* Tab System */}
        <div className="flex bg-zinc-950 border border-white/5 p-1 rounded-xl gap-1">
          <button 
            type="button"
            onClick={() => setActiveTab('local')}
            className={cn(
              "flex-1 py-1.5 text-[9px] uppercase tracking-widest font-black transition-all rounded-lg",
              activeTab === 'local' 
                ? "bg-zinc-900 text-white border border-white/10 shadow-md" 
          ...
          <button 
            ...
            className={cn(
              "flex-1 py-1.5 text-[9px] uppercase tracking-widest font-black transition-all rounded-lg flex items-center justify-center gap-1.5",
              activeTab === 'google' 
                ? "bg-zinc-900 text-emerald-400 border border-emerald-500/10 shadow-md" 
                : "bg-transparent border border-transparent text-zinc-500 hover:text-emerald-400"
            )}
          >
      ...
      {activeTab === 'google' ? (
        ...
        <>
          <div className="p-6 border-b border-white/5 bg-zinc-90 w-full shrink-0">
